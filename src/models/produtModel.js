...
const { createColum } = require('../utils/createColumnsAndPlaceholders');
...
const insert = async (newProduct) => {
  const { columns, placeholders } = createColum(newProduct);
...
const search = async (searchToy) => {
  const findBySearch = `%${searchToy}%`;
  const query = 'SELECT * FROM products WHERE name LIKE ?';
  const [products] = await connection.execute(query, [findBySearch]);
  return products;
};
